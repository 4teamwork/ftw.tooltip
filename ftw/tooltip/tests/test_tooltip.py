...
class TestTooltip(MockTestCase):
    ...
    def setUp(self):
        ...
        self.expect(self.response.getHeader('Content-Type')).result('text/javascript')
    ...
    def test_component_registered(self):
        obj = queryMultiAdapter((object(), object()), ITooltipSource, name="demo1")
    ...
    def test_get_all_tooltip(self):
        ...
        for name, adapter in view.get_all_tips():
            self.assertTrue(adapter.__class__.__name__ in ['DemoDynamicTooltipSource', 'DemoStaticTooltipSource'])
...
'condition': 'body'}]""")
    ...
    def test_tooltip_js_multiple_adapters_generation(self):
        # The text attr of the second adapter should be empty, because it uses the title attr
